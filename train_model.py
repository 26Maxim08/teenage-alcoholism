"""
Обучение модели для определения риска злоупотребления алкоголем у студентов.

Датасет: Student Alcohol Consumption (UCI), файл data/student-mat.csv
Задача:  бинарная классификация — "группа риска" vs "норма".

Целевая переменная строится из двух колонок опроса:
    Dalc - потребление алкоголя в будние дни (шкала 1..5)
    Walc - потребление алкоголя в выходные  (шкала 1..5)
Взвешиваем по числу дней в неделе:  weekly = Dalc*5 + Walc*2
Студент относится к ГРУППЕ РИСКА (класс 1), если weekly >= 14
(примерно верхняя треть выборки). Колонки Dalc и Walc после
формирования цели удаляются из признаков, чтобы исключить утечку.

Особенности задачи (скрининг):
  - пропустить студента из группы риска (FN) дороже, чем ложная тревога (FP),
    поэтому рабочий порог классификации подбирается под максимум F1
    по кросс-валидации на обучающей выборке, а не берётся равным 0.5.
"""
